...
The first command runs deterministic gesture and channel-rule checks without audio hardware. The second runs the updater test suite. The last two check release guards, scan failure handling and evidence generation using harmless fixtures; they do not build the app or installer.
...
Replace `1.0.1` with the version in `Directory.Build.props`. The release workflow validates relevant pull requests without publishing. On tag pushes it checks that the tag matches, builds and scans the self-contained x64 installer, and publishes it with its `.sha256` checksum and `.defender.json` evidence. The publishing job uses the repository's built-in Actions token.
