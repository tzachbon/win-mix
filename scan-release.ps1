...
if ($LogPath) { [IO.File]::WriteAllText([IO.Path]::GetFullPath($LogPath), '') }
...
& $Scanner -Scan -ScanType 3 -File $target.FullName -DisableRemediation 2>&1 | ForEach-Object {
    if ($LogPath) { $_ | Out-File -LiteralPath $LogPath -Append -Encoding utf8 }
    $_ | Out-Host
}
