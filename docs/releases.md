...
Run `node --test Tests/*.test.cjs`, `./Tests/ReleaseChecks.ps1`, `./Tests/DefenderChecks.ps1`, `./Tests/DefenderDiagnosticChecks.ps1`, the installer shutdown checks described in CONTRIBUTING.md, and `./build.ps1` on Windows with the pinned .NET SDK and Inno Setup. Builds and CI do not request Defender scans by default. Add `-ScanWithDefender` to explicitly scan the published application tree and final installer; failures still stop that opt-in build. CI retains only the installer and checksum as the `installer` artifact, preserving the two-asset publication contract. The optional `defender-evidence` artifact contains scan evidence only when a scan was requested. Policy and Defender tests use isolated fixtures, not live scans or publication.
